...
export function getApartments333Fremont(apartmentsData) {
  ...
  const apartments = apartmentsData.apartments.map((item) => {
    ...
    const area = areaInfo + " Sq. Ft.";
    const price = item.price ? item.price.split(" ")[0] : "";
    const apartment_link = item.link ?? "";
...
export function getApartmentsRinconGreen(apartmentsData) {
  ...
  const apartments = apartmentsData.apartments.map((item) => {
    ...
    const bathsInfo = item.baths ?? null;
    const baths =
      bathsInfo === "1" ? bathsInfo + " Bath" : bathsInfo + " Baths";
    const area = item.area ? item.area + " Sq. Ft." : null;
    const priceInfo = item.price ? item.price.split(".")[0] : null;
    var price = null;
    ...
    console.log(item.price, item.area);
    const image = item.picture ?? "";
    const linkInfo = item.link ? item.link.split(",") : null;
    const siteId = linkInfo ? linkInfo[1].trim() : null;
    const fId = linkInfo ? linkInfo[2].trim() : null;
    const apartment_link =
      siteId && fId
        ? `https://www.rincongreen.com/Brochure/Two-Bedroom?siteId=${siteId}&fId=${fId}`
        : "https://www.rincongreen.com/Floor-plans.aspx";
